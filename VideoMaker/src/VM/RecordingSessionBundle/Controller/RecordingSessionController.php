...
class RecordingSessionController extends Controller {
    ...
    public function foShowAction($slug_sess) {
        ...
        $cmd = 'ffmpeg -y -i '.$streamsPath.$request->get('filename').'.flv -s 640x480 -ar 44100 -pass 1 -b 1400k -r 30 -ab 128k -f avi '.$streamsPath.$request->get('filename').'.avi 2> '.$kernel->getRootDir().'/logs/encoder.txt';
