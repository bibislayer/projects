...
class RegistrationController extends Controller implements ContainerAwareInterface {
    ...
    public function registerAction(Request $request) {
        ...
        /** @var $formFactory \Poker\UserBundle\Form\Factory\FactoryInterface */
        $formFactory = $this->container->get('poker_user.registration.form.type');
        /** @var $dispatcher \Symfony\Component\EventDispatcher\EventDispatcherInterface */
        $dispatcher = $this->container->get('event_dispatcher');

        $userForm = $request->get($formFactory->getName()); //poker_user_registration
        ...
        if (is_array($userForm) && array_key_exists('user_profile', $userForm)) {
            ...
            ///SECTION A REVOIR/// 
            $q = $em->createQuery("SELECT MAX(u.username) as username FROM Poker\UserBundle\Entity\User u where u.username like '" . $username . "%'");
            $em->flush();
            $result = $q->getResult();
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if($inForm){
                    ...
                    if (null === $response = $event->getResponse()) {                   
                        return $this->render('PokerUserBundle:Registration:register_success.html.twig', $paramsView);
                    }
                    return $response;
                }                
            }
        }
        return $this->container->get('templating')->renderResponse('PokerUserBundle:Registration:register.html.' . $this->getEngine(), array(
    ...
    public function checkEmailAction() {
        ...
        return $this->container->get('templating')->renderResponse('PokerUserBundle:Registration:checkEmail.html.' . $this->getEngine(), array(
                    'user' => $user,
        ));
    }
    ...
    public function confirmedAction() {
        ...
        return $this->container->get('templating')->renderResponse('PokerUserBundle:Registration:confirmed.html.' . $this->getEngine(), array(
                    'user' => $user,
        ));
    }
