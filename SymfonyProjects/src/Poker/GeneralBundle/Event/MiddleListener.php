 ...
 */
namespace Poker\GeneralBundle\Event;
...
class MiddleListener
{
    ...
    private function checkMiddleRestrictions()
    {
        ...
        /*if (!in_array($this->route, array('mo_dashboard'))) {
            ...
            $ent = $this->container->get('doctrine')->getManager()->createQuery('SELECT e.id, e.logo, e.name, e.slug  FROM PokerEnterpriseBundle:Enterprise e WHERE e.slug = :slug')
