...
class LoginController extends Controller {
    ...
    public function foLoginAction() {
        ...
        return $this->render('FPUserBundle:Login/Front:login.html.twig', array(
     ...
     public function moLoginAction() {
        ...
        $csrfToken = $this->container->get('form.csrf_provider')->generateCsrfToken('authenticate');
        return $this->render('FPUserBundle:Login/Middle:login.html.twig', array(
            // last username entered by the user
            'last_username' => $session->get(SecurityContext::LAST_USERNAME),
            'error' => $error,
            'csrf_token' => $csrfToken
    ...
    public function boLoginAction() {
        ...
        $csrfToken = $this->container->get('form.csrf_provider')->generateCsrfToken('authenticate');
        return $this->render('FPUserBundle:Login/Back:login.html.twig', array(
            // last username entered by the user
            'last_username' => $session->get(SecurityContext::LAST_USERNAME),
            'error' => $error,
            'csrf_token' => $csrfToken
